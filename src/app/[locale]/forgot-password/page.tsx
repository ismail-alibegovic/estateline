...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error || 'Neuspešan zahtjev za ponovno postavljanje lozinke')
        return
      }

      setMessage(data.message || 'Upute su poslate na vašu email adresu.')
    } catch {
      ...
      <div className="w-full max-w-md bg-white border border-[#E8E2D6] rounded-3xl p-8 shadow-xl shadow-amber-950/5 space-y-6">
        <div className="text-center space-y-2">
          ...
          <p className="text-sm text-slate-600">
            Unesite vašu email adresu i poslaćemo vam link za poništavanje lozinke.
          </p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input
                ...
                placeholder="vaš.email@agencija.ba"
            ...
            >
              {loading ? 'Slanje u toku...' : 'Pošalji link za ponovno postavljanje'}
            </button>
          </form>
        )}
